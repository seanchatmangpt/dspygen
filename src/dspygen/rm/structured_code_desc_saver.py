...
def detect_language_and_extension(code: str) -> str:
    ...
    if "```javascript" in code or code.strip().startswith(("function", "var", "let", "const")):
        return LANGUAGE_EXTENSIONS['javascript']
    if "```java" in code or code.strip().startswith("public class"):
        return LANGUAGE_EXTENSIONS['java']
    if "```cpp" in code or code.strip().startswith("#include"):
        return LANGUAGE_EXTENSIONS['cpp']
    if "```html" in code or code.strip().startswith(("<!DOCTYPE html>", "<html>")):
        return LANGUAGE_EXTENSIONS['html']
    if "```css" in code or code.strip().startswith("body {") or code.strip().endswith("}"):
        return LANGUAGE_EXTENSIONS['css']
    if "```go" in code or code.strip().startswith("package main"):
        return LANGUAGE_EXTENSIONS['go']
    if "```php" in code or code.strip().startswith("<?php"):
        return LANGUAGE_EXTENSIONS['php']
    if "```sql" in code or code.strip().startswith(("SELECT", "INSERT", "UPDATE", "DELETE")):
        return LANGUAGE_EXTENSIONS['sql']
    if "```shell" in code or code.strip().startswith("#!/bin/bash"):
