...
def create_dynamic_signature_class(base_class_name, rfc, code_snippet):
    """
    Dynamically create a RFCforTask signature class.
    """
    class_dict = {
        "__doc__": f"RFC for change task {base_class_name} to modify code_snippet with a fix: {rfc} .",
        "task_description": dspy.InputField(desc=f"Description of the task to fix the code snippet with  RFC: {rfc}."),
        "rfc": dspy.InputField(desc=f"RFC: {rfc}"),
        "code_snippet_with_errors": dspy.InputField(desc=f"Code snippet needs the fix: ```{code_snippet}'''"),
        "code_snippet": dspy.OutputField(desc=f"Plain code_snippet having fix by RFC: {rfc}. No chat text outside allowed and do not repeate old code!"),
    }
    signature_class = type(f"RFCfor{base_class_name}", (dspy.Signature,), class_dict)
    return signature_class
