...
class GenerateJSONFromText(dspy.Signature):
    ...
    json_schema = dspy.InputField(
        desc="JSON schema to validate the JSON object. YOUR RESPONSE MUST ADHERE TO THIS SCHEMA.")
    text_information = dspy.InputField(
        desc="Text information in structured or unstructured format. To be converted into a JSON object.")

    # The output JSON object, as a string, based on the processed text information.
    json_object = dspy.OutputField(
        desc="YOUR ONLY OUTPUT IS THE JSON OBJECT. Ensure it adheres to the provided JSON schema. Do not include the schema in the output.")
...
@router.post("/json/")
async def json_route(data: dict):
    # Your code generation logic here
    from dspygen.lm.groq_lm import Groq
    init_dspy(Groq, 1000, "mixtral-8x7b-32768")

    print(data)
    return json_call(**data)
...
st.title("JsonModule Generator")
text = st.text_input("Enter text")

if st.button("Submit JsonModule"):
    init_dspy()

    result = json_call(text=text)
    st.write(result)
"""
